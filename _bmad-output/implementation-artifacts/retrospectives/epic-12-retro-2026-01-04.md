...
3. Story location mismatch (_bmad-output/implementation-artifacts/stories/ vs _bmad-output/)
...
### Retrieval Quality Enhancements (Party Mode Analysis 2026-01-04)

The following gaps were identified during a party mode analysis of retrieval capabilities:

5. **Retrieval Quality Benchmarks Framework**
   - Create evaluation dataset with labeled query-document pairs
   - Implement MRR@K (Mean Reciprocal Rank) metrics
   - Implement NDCG@K (Normalized Discounted Cumulative Gain)
   - Add Precision@K and Recall@K scoring
   - Enable A/B comparison framework (before/after reranking)
   - Rationale: Current benchmarks only measure latency, not retrieval quality

6. **Hierarchical Chunk Retrieval**
   - Add `parent_chunk_id` linking chunks to parent sections
   - Add `hierarchy_level` field (0=doc, 1=section, 2=chunk)
   - Implement "small-to-big" retrieval pattern (retrieve chunk → fetch parent context)
   - Support multi-level expansion (chunk → section → document)
   - Rationale: Current chunking captures section hierarchy but retrieval doesn't use it

7. **Prometheus Metrics for Retrieval**
   - Export `retrieval_precision` gauge
   - Export `retrieval_recall` gauge
   - Export `reranking_improvement` histogram (pre vs post rerank scores)
   - Export `grader_score` histogram with threshold bucketing
   - Export `fallback_triggered_total` counter by strategy
   - Rationale: No observability into retrieval quality beyond structured logs

8. **Grafana Retrieval Dashboards**
   - Create retrieval quality dashboard with precision/recall trends
   - Add reranking effectiveness visualization
   - Add grader score distribution charts
   - Include fallback trigger rate monitoring
   - Rationale: Ops team has no visibility into retrieval health
