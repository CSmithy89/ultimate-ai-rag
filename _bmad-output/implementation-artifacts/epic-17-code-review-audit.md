...
3. **Add profile schema validation** - Use pydantic or JSON schema to validate profile YAML
4. **Implement real version checking** - Query PyPI/GitHub for latest version in `update check`
5. **Add CLI help text** - Add `--help` descriptions for all commands and options

### Nice to Have

6. **Add progress spinners** - Use rich.progress for docker compose and service wait
7. **Add shell completion** - typer supports bash/zsh/fish completion generation
8. **Add verbose mode** - `--verbose` flag for detailed logging
